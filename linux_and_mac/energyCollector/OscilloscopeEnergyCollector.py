...
import sys
from linux_and_mac.energyCollector.AbstractEnergyCollector import AbstractEnergyCollector
...
class OscilloscopeEnergyCollector(AbstractEnergyCollector):

    def __init__(self):
        self.func_data_ready = CALLBACK_DATA_READY(self.myfunction1)
        self.func_data_overflow = CALLBACK_DATA_OVERFLOW(self.myfunction2)
        self.frequency = 125000  # Frequency to be set in the oscilloscope to measure
        self.dataRead = []
    ...
    def stop(self):
        ...
        if self.USING_UCURRENT_DEVICE:
            powerTraceGenerated = self.saveDataToFileCalculatingPowerUsinguCurrent(self.ouputEnergyFileName, dataRead, self.TIME_INIT, self.frequency)
        else:
            powerTraceGenerated = self.saveDataToFileCalculatingPowerUsingAmplifier(self.ouputEnergyFileName, dataRead, self.TIME_INIT, self.frequencyf,
