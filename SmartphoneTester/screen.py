...
class screen():
    ...
    def __init__(self, api, screen_width, screen_height, fenetre):
        ...
        self.Hori_distance = math.sqrt(
            (self.topleft[0] - self.topright[0]) ** 2 + (self.topleft[1] - self.topright[1]) ** 2)
        self.Vert_distance = math.sqrt(
            (self.topleft[0] - self.bottomleft[0]) ** 2 + (self.topleft[1] - self.bottomleft[1]) ** 2)
    ...
    def Calc_Coordinates(self, x_pixel, y_pixel):
        ...
        y3 = self.topleft[1]
        # calcul de l'angle
        angle = math.acos(((x1 - x3) * (x2 - x3) + (y1 - y3) * (y2 - y3)) / (
