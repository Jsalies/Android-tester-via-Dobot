...
flag = False
test_duration = 0
def start_test_case(package):
	global flag
	global test_duration
	# Start the test case
	print("Test case start running")
	start = time.time()
 	robotiumOutput = check_output(ADB_PATH + " shell am instrument -e class " + str(package) + ".test.Scenario1 -w " + str(package) + ".test/android.test.InstrumentationTestRunner", shell=True, universal_newlines=True)
 	test_duration = time.time() - start
 	#subprocess.call(ADB_PATH + " disconnect " + device_ip, shell=True, universal_newlines=True)
 	flag = True
 	time.sleep(10)
 	print("Test case finish running")
 	print "reset devices"
 	print test_duration
 	# Stop test case 
 	reset_device(robotiumOutput,package)
  ...
  def __init__(self, device=None, serialno=None, wait=1):
    ...
    while True:  # try all /dev/ttyACM* until we find one we can use
      for dev in os.listdir("/dev"):
        ...
        if not dev.startswith("tty.usbmodem1421"): continue
	...
	if FLAGS.samples:
		...
		for iteration in range(0, RUNS):
			...
			for file in sorted(os.listdir(APKS_FOLDER)):
				...
				# Install the apk and the apk containing testing case (preparing the testing case)
				
				package = prepare_test_case(file)
				...
				### Starts case study blocks the script: put it on another thread => To check and do ###############
				
				
				subprocess.call(ADB_PATH + " tcpip "+"5555", shell=True, universal_newlines=True)
				connect_Output = check_output(ADB_PATH + " connect " + device_ip, shell=True, universal_newlines=True)
				print("OUTPUT: " + connect_Output)
				mon.SetUsbPassthrough(0)
				time.sleep(10)
				subprocess.call(ADB_PATH + " devices", shell=True, universal_newlines=True)
				Thread_run_test_case = threading.Thread(target=start_test_case, args=(package,))
				Thread_run_test_case.start()
		
				# Start Data_collection: Collect and average samples as specified
				...
				with open (filePath, 'a') as fd:
					...
					try:
						...
						while ((emitted < FLAGS.samples or FLAGS.samples == -1) and flag == False):
						
							#if not Thread_run_test_case.isAlive():
								#flag =True
								#Thread_run_test_case.join()
							...
							if need > len(collected):     # still need more input samples
								...
								##### if the start test case thread joined  then Stop Data_collection and break the while loop ###########
					except KeyboardInterrupt:
						print >>sys.stderr, "interrupted"
					sys.stdout = old_stdout
				
				print "Stop collecting energy"
				# Stop Data_collection
				mon.StopDataCollection()
				subprocess.call(ADB_PATH + " disconnect " + device_ip, shell=True, universal_newlines=True)
				mon.SetUsbPassthrough(1)
				subprocess.call(ADB_PATH + " devices", shell=True, universal_newlines=True)
