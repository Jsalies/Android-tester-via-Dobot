...
def start_test_case(package, width, height):
	global flag
	global test_duration
	# Start the test case
	print("Test case start running")
	start = time.time()
 	subprocess.call("python DobotControl.py --package-simulation=" + package + " --screen-width=" + str(width) + " --screen-height=" + str(height), shell=True, universal_newlines=True)
  ...
  def __init__(self, device=None, serialno=None, wait=1):
    ...
    while True:  # try all /dev/ttyACM* until we find one we can use
      for dev in os.listdir("/dev"):
        ...
        if not dev.startswith("tty.usbmodem1411"): continue
	...
	if FLAGS.samples:
		...
		for iteration in range(0, RUNS):
			...
			for file in sorted(os.listdir(APKS_FOLDER)):
				...
				Thread_run_test_case = threading.Thread(target=start_test_case, args=(package,width,height))
